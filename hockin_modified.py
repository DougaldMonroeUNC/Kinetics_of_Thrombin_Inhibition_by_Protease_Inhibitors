...
def setIC(ICvector = np.array([10e-12, 1.45e-6, 2e-8, 1e-8, 1e-8/100, 7e-10, 9e-8, 1.6e-7, 3e-8, 3.5e-6, 2.5e-9, 1.2e-6, 3.2e-5, 2.4e-6]), includeExtras = False):
    """
    Converts a list of factor levels into an initial condition vector for the Hockin model assuming 0 concentration for all remaining species. \n
    Inputs: \n
    ...
    """
    IC = np.zeros(43);
    IC[[0,13,20,1,3,14,10,7,28,25,34,35,36]] = np.delete(ICvector,8); # np.delete removes XI from the input. Indices correspond to factors in initial condition vector
    return IC
